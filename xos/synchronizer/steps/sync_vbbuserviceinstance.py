...
class SyncVBBUServiceInstance(SyncInstanceUsingAnsible):

    provides = [VBBUServiceInstance]

    observes = VBBUServiceInstance

    requested_interval = 0

    template_name = "vbbuserviceinstance_playbook.yaml"

    service_key_name = "/opt/xos/synchronizers/vbbuservice/vbbuservice_private_key"
    ...
    def __init__(self, *args, **kwargs):
        super(SyncVBBUServiceInstance, self).__init__(*args, **kwargs)

    def get_vbbuservice(self, o):
        if not o.owner:
            return None

        vbbuservice = VBBUService.objects.filter(id=o.owner.id)

        if not vbbuservice:
            return None

        return vbbuservice[0]
    ...
    def get_extra_attributes(self, o):
        ...
        vbbuservice = self.get_vbbuservice(o)
        fields['service_message'] = vbbuservice.service_message
    ...
    def delete_record(self, port):
        # Nothing needs to be done to delete an vbbuservice; it goes away
        # when the instance holding the vbbuservice is deleted.
        pass
    ...
    def handle_service_monitoringagentinfo_watch_notification(self, monitoring_agent_info):
        ...
        objs = VBBUServiceInstance.objects.all()
        for obj in objs:
            ...
            logger.info("handling watch notification for monitoring agent info:%s for VBBUServiceInstance object:%s" % (monitoring_agent_info, obj))
            ...
            super(SyncVBBUServiceInstance, self).run_playbook(obj, fields, template_name)
        pass
